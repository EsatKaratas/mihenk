...
| **Sürüm geçişlerinde veri güvenliği** (§50) | Kayıtlı kullanıcı verisi sürüm değişikliğinde kaybolmaz. Atölyeler *sınıf* ekseninden müfredat eksenine taşınırken, daha önce siteyi açmış tarayıcılarda o kazanımların görünmez kalma riski **önceden ölçüldü** ve iki göç yazıldı: biri eski değeri doğru alana taşır, diğeri sürümle gelen yeni kazanımları kayıtlı listeye **yalnızca ekler** (kullanıcının sildiğini geri getirmez). Canlıda gerçek bayat kayıtla ölçüldü. |
| **Öğrenci verisi artık sunucuya hiç gitmiyor** (§43) | Cihazlar arası paylaşım bir *oda koduna* dayanıyordu; oda kodu kimlik doğrulama değildir, bu yüzden özellik uçlarıyla ve veritabanı bağlamasıyla birlikte **tamamen söküldü**. Bugün sınav, yanıt, öğrenci adı ve puan hiçbir koşulda sunucuya gitmez — yalnızca yapay zekâ çağrısındaki metin iletilir. **Bedeli açıkça yazıldı:** ürün artık tek cihazda çalışır. |
| **Otomatik yedek model devrede** (§43) | Birincil model çekilir ya da hata verirse ikinci model otomatik devreye giriyor. Yedek eskiden tanımlıydı ama devreye giremiyordu; çözüm anahtar almak değil kodu okumak oldu — ayrıntısı [§3.1](#31-tek-sağlayıcıya-bağımlı-değil--otomatik-yedek)'de. |
| **Sınav yayın kapısı sertleştirildi** (§44) | Rubrik geçerliliği tek bir ölçüte bağlandı: kriter adı boşsa ya da ağırlık toplamı %100 değilse sınav yayınlanamaz ve **şerit sebebini yazar**. Öncesinde bu ölçüt iki ayrı yerde yazılıydı ve hata puanlama anına kadar görünmüyordu. |
| **Tekrar eleme + birim dönüşümü sertleştirmesi** (§46) | İki kusur sahadan geldi. Biri: gövdesi farklı ama **şıkları aynı** iki soru — tekrar denetimi yalnızca gövdeye bakıyordu, artık şık kümesinin **sıradan bağımsız imzasına** da bakıyor. Diğeri: *"1 kilogram su kaç santilitre?"* sorusunda cevap anahtarı yanlıştı (1000 yerine 100). Olgusal hatayı hiçbir şema doğrulaması yakalayamaz — **onay ekranı yakaladı**; yine de istem sertleştirildi ve A/B ile ölçüldü: eski istem **0/2** doğru, yeni istem **4/4**. |
...
| **Yabancı alfabe denetimi genişletildi** (§48) | `dilUyarisi` yalnızca Latin **dışı** alfabeleri tarıyordu; canlıda üretilen `phứcekli` (Vietnamca `ứ`) sessizce geçti. Aralık eklendi — Türkçenin `ğıİşŞĞ` harfleri başka bir blokta olduğu için yanlış pozitif riski yok. Ayrıca `İzmir` kelimesinin anahtar kelime çıkarımında `Zmir`e dönüştüğü bulundu ve düzeltildi. |
| **Öğrenciye gidecek metin de denetleniyor** (§45) | Yabancı alfabe koruması yalnızca soru üretimindeydi. Modelin **öğrenciye gidecek** geri bildirim taslağına bir CJK karakteri sızdığı ölçüldükten sonra denetim değerlendirme çıktısına da eklendi ve testle kilitlendi. |
